...
def create_note():
    ...
    while True:
        status = input(
            f"Введите статус заметки "
            f"(новая, в процессе, выполнено): "
        ).strip().lower()
        if status in ["новая", "в процессе", "выполнено"]:
            ...
            print(
                f"Некорректный статус. Пожалуйста, введите 'новая', "
                f"'в процессе' или 'выполнено'."
            )
    ...
    note["issue_date"] = input_date(
        f"Введите дату истечения заметки в формате 'ДД-ММ-ГГГГ': "
    )
    return note
