...
def create_note():
    ...
    while True:
        ...
        if title in note["titles"]:
            ...
            note["titles"].append(title)  # Добавление уникального заголовка в список
    ...
    if confirm("Хотите обновить статус заметки? (да/нет): "):
        update_note_status(note)  # Вызов функции для обновления статуса заметки

    # Проверка корректного ввода даты создания заметки
    note["created_date"] = input_date("Введите дату создания заметки в формате 'ДД-ММ-ГГГГ': ")
    ...
    print("\nЗаметка успешно добавлена!")  # Подтверждение успешного добавления заметки
...
def update_note_status(note):
    valid_statuses = {"1": "выполнено", "2": "в процессе", "3": "отложено"}  # Словарь с допустимыми статусами
    while True:
        display_status_options()  # Вывод доступных статусов
        choice = input("Введите номер статуса либо нажмите Ввод чтобы оставить прежний статус: ")  # Ввод статуса
        if choice in valid_statuses:
            note["status"] = valid_statuses[choice]  # Обновление статуса заметки
            print("Статус заметки успешно обновлён на:", note["status"])  # Подтверждение обновления статуса
            break
        elif choice == "":
            break  # Выход из цикла, если пользователь оставил строку пустой
        else:
            print("Некорректный ввод. Пожалуйста, выберите номер из предложенных вариантов.")  # Обработка некорректного ввода

# Функция проверки дедлайна
def check_deadline(note):
    current_date = datetime.now()  # Получение текущей даты
    print(f"Текущая дата: {current_date.strftime('%d-%m-%Y')}")  # Вывод текущей даты
    while True:
        try:
            deadline = datetime.strptime(note["issue_date"], "%d-%m-%Y")
            days_left = (deadline - current_date).days  # Вычисление количества дней до дедлайна
...
def display_notes():
    ...
    for i, note in enumerate(notes, start=1):  # Перебор всех заметок в списке
        ...
        for j, title in enumerate(note["titles"], start=1):  # Перебор заголовков заметки
...
# Точка входа и запуск основной функции
