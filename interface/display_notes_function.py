...
# Функция для отображения одной заметки
def display_note(note, index):
    ...
    print(
        Fore.LIGHTWHITE_EX + "Имя пользователя: " +
    ...
    for j, title in enumerate(note["titles"], start=1):
        print(
            Fore.LIGHTWHITE_EX +
            f'- Заголовок № {j}: {Style.BRIGHT + Fore.WHITE}{title}'
        )

    print(
        Fore.LIGHTWHITE_EX + "Описание: " +
        Style.BRIGHT + Fore.WHITE + note["content"]
    )
    print(
        Fore.LIGHTWHITE_EX + "Статус: " +
        Style.BRIGHT + Fore.WHITE + note["status"]
    )
    print(
        Fore.LIGHTWHITE_EX + 'Дата создания: ' +
        Style.BRIGHT + Fore.WHITE + note['created_date']
    )
    print(
        Fore.LIGHTWHITE_EX + 'Дедлайн: ' +
